...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for problem in orm.Problem.objects.all():
            for feature in problem.features.split(','):
                feature = feature.strip().lower()
                if feature == '':
                    continue
                if feature in ['crack', 'crimp', 'jug', 'sloper', 'dyno', 
                               'sidepull']:
                    feature += 's'
                elif feature == 'pinch':
                    feature += 'es'
                setattr(problem, feature, True)
            problem.save()

    def backwards(self, orm):
        for problem in orm.Problem.objects.all():
            feature_list = ['cracks', 'crimps', 'jugs', 'slopers', 'dynos',
                            'sidepulls', 'compression', 'highball', 'mantle',
                            'pockets', 'pinches', 'arete']
            this_problems_features = []
            for feature in feature_list:
                if getattr(problem, feature):
                    this_problems_features.append(feature)
            problem.features = ', '.join(this_problems_features)
            problem.save()
            this_problems_features = []
